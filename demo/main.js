...
var DisplacementLayer = exports.DisplacementLayer = function () {
	function DisplacementLayer(element) {
		...
		_lodash2.default.extend(this, {
			...
			displacement: new _vector.Vector(0, 0)
		});
	}

	_createClass(DisplacementLayer, [{
		key: 'displaceBy',
		value: function displaceBy(changeVector) {
...
var PathTweener = exports.PathTweener = function () {
	function PathTweener(element, path) {
		_classCallCheck(this, PathTweener);

		var pathElem = path[0];
		var pathStartVector = this.pointToVector(pathElem.getPointAtLength(0));

		_lodash2.default.extend(this, {
			element: element,
			path: pathElem,
			pathLength: pathElem.getTotalLength(),
			pathStartVector: pathStartVector
		});
	}

	_createClass(PathTweener, [{
		key: 'pointToVector',
		value: function pointToVector(point) {
			return new _vector.Vector(point.x, point.y);
		}
	}, {
		key: 'update',
		value: function update(percentage) {
			var distance = percentage * this.pathLength;
			var pathDestVector = this.pointToVector(this.path.getPointAtLength(distance)).sub(this.pathStartVector);
...
var ScrollHeightTweener = exports.ScrollHeightTweener = function () {
	...
	_createClass(ScrollHeightTweener, [{
		key: 'setCurrentHeight',
		value: function setCurrentHeight(currentHeight) {
			var percentage = 0;
...
var Vector = exports.Vector = function () {
	...
	_createClass(Vector, [{
		...
		key: 'vectorScale',
		value: function vectorScale(rhs) {
			return new Vector(this.left * rhs.left, this.top * rhs.top);
		}
	}]);
...
var _vector = require('./engine/vector');

var _displacementLayer = require('./engine/displacement-layer');

var _pathTweener = require('./engine/path-tweener');

var _scrollHeightTweener = require('./engine/scroll-height-tweener');

//import {MouseMoveEventHandler} from './engine/mouse-move-event-handler';

$(function () {
		$('#parallax-demo').load(function () {
				var svgRoot = $('object').contents().find('svg');
				...
				var bigClouds = new _displacementLayer.DisplacementLayer(svgRoot.find('#big_clouds'), new _vector.Vector(1, 0.5));
				var mediumClouds = new _displacementLayer.DisplacementLayer(svgRoot.find('#clouds_medium'), new _vector.Vector(1, 0.2));
				var smallClouds = new _displacementLayer.DisplacementLayer(svgRoot.find('#small_clouds'), new _vector.Vector(1, 0.1));

				var balloonPink = new _displacementLayer.DisplacementLayer(svgRoot.find('#ballon_pink'), new _vector.Vector(1, 0.15));
				var balloonYellow = new _displacementLayer.DisplacementLayer(svgRoot.find('#ballon_yellow'), new _vector.Vector(1, 0.3));
				...
				var oldScrollVector = new _vector.Vector(0, 0);
...
},{"./engine/displacement-layer":2,"./engine/path-tweener":3,"./engine/scroll-height-tweener":4,"./engine/vector":5}]},{},[6]);
