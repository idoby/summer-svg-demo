...
//		}));
		...
		const sunHeightTweener = new ScrollHeightTweener(sun, 337, 3380);

		const airplane = new PathTweener(svgRoot.find('#airplane'), svgRoot.find('#airplane_path'));
		const airplaneHeightTweener = new ScrollHeightTweener(airplane, 1925, 2670);
		...
		window.addEventListener('scroll', () => {
			requestAnimationFrame(() => {
				...
				scrollHeightElements.forEach(tweener => tweener.setCurrentHeight(window.scrollY));
			});
		});
	});
});
